# encoding: UTF-8
# Cookbook Name:: et_apache_kafka
# Recipe:: configure
#

[
  node["et_apache_kafka"]["config_dir"],
  node["et_apache_kafka"]["bin_dir"],
  node["et_apache_kafka"]["data_dir"],
  node["et_apache_kafka"]["log_dir"]
].each do |dir|
  directory dir do
    recursive true
    owner node["et_apache_kafka"]["user"]
  end
end

%w{ kafka-server-start.sh kafka-run-class.sh kafka-topics.sh }.each do |bin|
  template ::File.join(node["et_apache_kafka"]["bin_dir"], bin) do
    source "bin/#{bin}.erb"
    owner "kafka"
    action :create
    mode "0755"
    variables(
      :config_dir => node["et_apache_kafka"]["config_dir"],
      :bin_dir => node["et_apache_kafka"]["bin_dir"]
    )
    notifies :restart, "service[kafka]", :delayed
  end
end

template ::File.join(node["et_apache_kafka"]["config_dir"],
                     node["et_apache_kafka"]["conf"]["server"]["file"]) do
  source "properties/server.properties.erb"
  owner "kafka"
  action :create
  mode "0644"
  variables(
    :broker_id => node["et_apache_kafka"]["broker.id"] || 0,
    :port => node["et_apache_kafka"]["port"],
    :zookeeper_connect => node["et_apache_kafka"]["zookeeper.connect"],
    :log_dirs => node["et_apache_kafka"]["data_dir"],
    :entries => node["et_apache_kafka"]["conf"]["server"]["entries"]
  )
  notifies :restart, "service[kafka]", :delayed
end

template ::File.join(node["et_apache_kafka"]["config_dir"],
                     node["et_apache_kafka"]["conf"]["log4j"]["file"]) do
  source "properties/log4j.properties.erb"
  owner "kafka"
  action :create
  mode "0644"
  variables(
    :log_dir => node["et_apache_kafka"]["log_dir"],
    :entries => node["et_apache_kafka"]["conf"]["log4j"]["entries"]
  )
  notifies :restart, "service[kafka]", :delayed
end
